...
export const PUT = async (req: NextRequest) => {
  ...
  if (comment.author_id === payload.uid) return NextResponse.json('您不能给自己点赞')

  const existing = await prisma.course_comment_like.findUnique({
    where: { user_id_comment_id: { user_id: payload.uid, comment_id: input.commentId } }
  })

  if (existing) {
    await prisma.course_comment_like.delete({ where: { user_id_comment_id: { user_id: payload.uid, comment_id: input.commentId } } })
    return NextResponse.json(false)
  } else {
    await prisma.course_comment_like.create({ data: { user_id: payload.uid, comment_id: input.commentId } })
