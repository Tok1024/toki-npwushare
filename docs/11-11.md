...
- **品牌重塑**：站点定位为「NWPUShare」，主页 Hero、导航、资源类型、README 等均更新为学习场景描述。Galgame 区块已移除或迁出首屏。
...
| `/course` _(待实现)_                  | 课程索引（列表 + 搜索 + 筛选）                   | 计划复用 `HomeCourseSection` 逻辑                        |
| `/department/[slug]` _(待实现)_       | 学院视图，列出课程/教师/资源统计                 | 待定义                                                   |
...
7. **后台任务**：`server/cron.ts` 仍在执行 TouchGal 任务（贡献值重置等），需替换为课程域的统计或清理任务。
