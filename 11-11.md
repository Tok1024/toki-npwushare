...
- **课程域模型落地**：Prisma 拆分出 department / teacher / course / course_teacher / resource / post / comment / course_rating 等表；`scripts/seedCourses.ts` 可注入示例学院/课程/资源，默认管理员 `seed@example.com / 123`。
- **课程详情页**：复用了 TouchGal 信息卡 + Tabs 结构，Tabs 包含「课程信息 / 课程资源 / 讨论版 / 课程评价」，并以 `/api/course/**` 数据驱动。评论/评分 UI 已接通课程 API。
...
| `course_rating` | 课程评分 | 推荐/评分/完成度/剧透字段、`course_rating_like` |
| `user` | 账号 | JWT 登录 + 2FA 字段；新增 `course_resource`、`course_rating_like`、`course_comment_like` 等关联 |
...
- 内容：为每门课程写入多条资源（slides/note/exam/solution/link 等类型）、帖子（Markdown 文本）、课程评论与评分，以及示例资源评论。
