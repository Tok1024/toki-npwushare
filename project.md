...
const created = await prisma.resource.create({ data: { title, type, tags, authorId } })
```
...
import { prisma } from '~/prisma'
export const auth = createBetterAuth({ adapter: prismaAdapter(prisma), session: { strategy: 'jwt', cookieName: 'kun-auth' }, providers: { email: { magicLink: true } } })
```
...
export const middleware = async (request: NextRequest) => {
  ...
  const needAuth = ['/admin', '/user'].some(p => request.nextUrl.pathname.startsWith(p))
  if (needAuth && !session) return NextResponse.redirect(new URL('/login', request.url))
