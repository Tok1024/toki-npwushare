...
import { FeedbackInteraction } from './feedback/InlineVote'
import { CourseFavoriteButton } from './FavoriteButton'
  ...
  return (
    <div className="space-y-6">
      <Card className="border-none shadow-medium bg-white/80 backdrop-blur-md">
        <CardBody className="flex flex-col gap-6 p-6 sm:p-8">
          <div className="flex flex-col md:flex-row md:items-start md:justify-between gap-4">
            ...
            <div className="flex items-center gap-3 md:justify-end flex-wrap md:flex-nowrap">
              {teachers?.length > 0 && (
              ...
              <CourseFavoriteButton courseId={course.id} showLabel />
            </div>
          </div>
          ...
          <div className="space-y-4 pt-4 border-t border-slate-100">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
              ...
              {/* 右侧：难度投票，sm+ 同行，xs 自动换行到下方 */}
              <div className="sm:ml-auto">
                <FeedbackInteraction dept={dept} slug={slug} courseId={course.id} />
              </div>
            </div>
          </div>
        </CardBody>
      ...
      >
        ...
        <Tab key="posts" title="经验帖" className="p-0 min-w-20">
          <div className="flex items-center justify-between mb-4">
            <div className="text-sm text-default-500">课程经验、复习要点、资料整理</div>
            <Button as={Link} href={`/course/${dept}/${slug}/p/new`} size="sm" color="primary">
              写经验贴
            </Button>
          </div>
          <PostList dept={dept} slug={slug} />
        </Tab>
